...
'''
cz_date = "12. 10. 2020"
print(f"{cz_date.split('. ')[2]}-{cz_date.split('. ')[1]}-{cz_date.split('. ')[0]}\n")

def txt_to_var_name(txt, l):
    if l.lower() == "python":
        return txt.lower().replace(" ", "_")
    else:
        return txt.split(" ")[0].lower() + txt[len(txt.split(" ")[0]):].title().replace(" ", "")

txt = "To je proměnná v Pythonu"

print(txt_to_var_name(txt, "Python"))
print(txt_to_var_name(txt, "js"))
...
def psswd_generator(num):
    for i in range(num):
        print(cycle(ascii_uppercase, 3) + cycle(ascii_lowercase, 3) + cycle("-/+*", 1) + cycle("0123456789", 3))
    return
