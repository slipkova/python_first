...
#? a) typ objektu - type()
#? b) identitu objektu (jeho adresu v paměti) - hex(id(id_objektu))
...
print(hex(id(students_count)), hex(id(rating)), hex(id(is_published)))
...
# &, |, ^, ~, <<, >>
vowels = "aeiouěýáíéúůó"
def bin_from_name(name):
    vowels = "aeiouěýáíéúůó"
    myself_binary = ''
    for i in name.replace(' ', '')[0:8].lower():
        if i in vowels:
            myself_binary += '0'
        else:
            myself_binary += '1'
    return myself_binary

myself_binary = int(bin_from_name("Eliška Slípková"), 2)
print(myself_binary)
print(myself_binary >> 2)
print(f"Binární součin čísel {bin(myself_binary)} a {bin(int('1A', 16))} je {bin(myself_binary * int('1A', 16))}")
